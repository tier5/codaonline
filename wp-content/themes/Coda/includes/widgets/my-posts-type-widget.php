...
function widget($args, $instance) {
  ...
  $cat_posts = new WP_Query($args);
	...
	<?php if ($instance['posttype'] == "testi") {
		...
		if ($instance['posttype'] == "services") {
		 	$custom = get_post_custom($post->ID);
			$hoverimg = $custom["hover-image"][0];
		}
	?>
			...
			<?php if ($instance["thumb"]) : ?>
			  ...
			  <?php if ( $instance['thumb_as_link'] ) : ?>
					...
					<?php if($instance['thumb_w']!=="" || $instance['thumb_h']!==""){ ?>
						...
						<?php if ($hoverimage!=="") {?>
							<span style="background-image:url(<?php echo $hoverimg; ?>); width:<?php echo $instance['thumb_w']; ?>px; height:<?php echo $instance['thumb_h']; ?>px"></span>
						<?php } ?>
					<?php }else{?>
