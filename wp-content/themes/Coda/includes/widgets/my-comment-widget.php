...
class MY_CommentWidget extends WP_Widget_Recent_Comments {
	...
	function MY_CommentWidget() {
		$widget_ops = array('classname' => 'widget_my_recent_comments', 'description' => __('My - Recent Comments','theme1669') );
		$this->WP_Widget('my-recent-comments', __('My - Recent Comments','theme1669'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('My Recent Comments','theme1669') : $instance['title']);
...
?>
      ...
      <li class="recentcomments">
        ...
        <div class="comment-body">
          <a href="<?php echo get_comment_link( $comment->comment_ID ); ?>" title="<?php _e('Go to this comment', 'theme1669'); ?>"><?php echo strip_tags(substr(apply_filters('get_comment_text', $comment->comment_content), 0, $comment_len)); if (strlen($comment->comment_content) > $comment_len) echo '...';?></a>
        </div>
			</li>
            <?php
			endforeach; endif;?></ul>
